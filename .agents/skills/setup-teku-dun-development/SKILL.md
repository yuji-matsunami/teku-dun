...
Android実機・emulatorを使う依頼では、`mobile/app` で `fvm flutter devices --machine` を実行し、emulatorが必要なら `fvm flutter emulators` で候補を確認する。候補が複数あり、依頼から対象を決められない場合はIDをユーザーに確認する。未起動emulatorは既存候補だけを起動し、AVDは作成しない。起動後、対象がdevices一覧に現れたことを確認する。

`task dev:check PROFILE=android` が `ANDROID_LICENSES_REQUIRED` を報告した場合は、Android SDKが提示するlicenseを承諾してよいかユーザーへ明示的に確認する。同意を得た場合だけ `task dev:accept-android-licenses CONFIRM_ANDROID_LICENSES=1` を実行し、完了後にcheckを再実行する。同意前、拒否された場合、応答が曖昧な場合は実行しない。Android CLIがlicense承諾操作は不要と正常終了した場合は、追加の同意を求めずcheck結果に従う。
...
Taskの前提条件やprofileの検証に失敗したら、下位コマンドへ切り替えず停止する。`ANDROID_LICENSES_REQUIRED` だけは上記の確認フローを使う。不足しているツールや確認が必要な端末を示し、ユーザーが環境を整えた後に再開できるよう失敗したTaskとstepを報告する。SDK解決を伴う診断では不足SDKのdownloadが起きる場合がある。
